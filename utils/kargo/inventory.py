...
class KargoInventory(object):
    ...
    def __init__(self, changed_hosts, config_file):
        self.config = configparser.ConfigParser(allow_no_value=True,
                                                delimiters=('\t', ' '))
        self.config.read(config_file)

        self.ensure_required_groups(ROLES)
        hosts = self.build_hostnames(changed_hosts)
        self.purge_invalid_hosts(hosts.keys(), PROTECTED_NAMES)
        self.set_kube_masters(list(hosts.keys())[:2])
        self.set_all(hosts)
        self.set_k8s_cluster()
        self.set_kube_node(hosts.keys())
        self.set_etcd(list(hosts.keys())[:3])
    ...
    def strip_opts(self, entry):
        self.debug(entry)
        return entry.split('\t', 1)[0]

    def get_ip_from_opts(self, optstring):
        print(optstring)
        opts = optstring.strip('[]').split(',')
        for opt in opts:
            k, v = opt.split('=')
            if k == "ip":
                return v
        raise Exception("IP parameter not found in options")
    ...
    def get_host_id(self, host):
        return int(re.findall("\d+", host)[-1])

    def build_hostnames(self, changed_hosts=None):
        ...
        for host in changed_hosts:
            if host[0] == "-":
                realhost = host[1:]
                if self.find_host_by_hostname(all_hosts, realhost):
                    self.debug("Marked {0} for deletion.".format(realhost))
                    all_hosts.remove(realhost)
                elif self.find_host_by_ip(all_hosts, realhost):
                    self.debug("Marked {0} for deletion.".format(realhost))
                    self.delete_host_by_ip(all_hosts, realhost)
            elif host[0].isdigit():
                if self.find_host_by_hostname(all_hosts, host):
                    self.debug("Skipping existing host {0}.".format(host))
                    continue
                elif self.find_host_by_ip(all_hosts, host):
                    self.debug("Skipping existing host {0}.".format(host))
                    continue
    ...
    def find_host_by_hostname(self, existing_hosts, hostname):
        return hostname in existing_hosts.keys()

    def find_host_by_ip(self, existing_hosts, ip):
    ...
    def set_kube_masters(self, hosts):
        for host in hosts:
...
def main(argv=None):
    if not argv:
        argv = sys.argv[1:]
    KargoInventory(argv, './inventory.cfg')

...
if ! admin_node_command type ansible > /dev/null || \
    ...
    while admin_node_command pgrep -f apt; do echo 'Waiting for apt lock...'; sleep 30; done
    echo "START DEBUG APT INFO"
    admin_node_command date
    admin_node_command ps waux | grep apt
    admin_node_command ls -l /var/lib/dpkg
    admin_node_command sudo apt-get check || true

    admin_node_command sleep 60
    admin_node_command ps waux | grep apt
    admin_node_command ls -l /var/lib/dpkg
    admin_node_command sudo apt-get check || true
    admin_node_command sudo lsof /var/lib/dpkg/lock
    admin_node_command date

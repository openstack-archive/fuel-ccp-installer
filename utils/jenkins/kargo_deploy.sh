...
if [[ "$ADMIN_IP" != "local" ]]; then
    echo "Preparing non-local admin node..."
    sshpass -p $ADMIN_PASSWORD ssh-copy-id $SSH_OPTIONS_COPYID -o PreferredAuthentications=password $ADMIN_USER@${ADMIN_IP} -p 22
    admin_node_command mkdir -p fuel-ccp-installer/utils/kargo
    tar cz ${BASH_SOURCE%/*}../kargo/* | admin_node_command tar xz - -C fuel-ccp-installer/utils/kargo
fi
...
admin_node_command CONFIG_FILE=kargo/inventory/inventory.cfg python3 ${BASH_SOURCE%/*}/../kargo/inventory.py ${SLAVE_IPS[@]}
...
until admin_node_command /usr/bin/ansible-playbook \
    ...
    ${BASH_SOURCE%/*}/../kargo/preinstall.yml $COMMON_DEFAULTS_OPT \
...
until admin_node_command /usr/bin/ansible-playbook \
    --ssh-extra-args "-o\ StrictHostKeyChecking=no" -u ${ADMIN_USER} -b \
    --become-user=root -i /home/${ADMIN_USER}/kargo/inventory/inventory.cfg \
    ${BASH_SOURCE%/*}/../kargo/postinstall.yml $COMMON_DEFAULTS_OPT \
    $OS_SPECIFIC_DEFAULTS_OPT $custom_opts; do
        if [[ $tries > 0 ]]; then
            (( tries-- ))
            echo "Deployment failed! Trying $tries more times..."

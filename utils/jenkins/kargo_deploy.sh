...
if ! admin_node_command type ansible > /dev/null || \
    ...
    with_retries admin_node_command -- "wget -N http://ppa.launchpad.net/ansible/ansible/ubuntu/pool/main/a/ansible/ansible_2.2.0.0-1ppa~xenial_all.deb && sudo dpkg -i ansible_2.2.0.0-1ppa~xenial_all.deb || sudo apt-get -f install -y"
fi

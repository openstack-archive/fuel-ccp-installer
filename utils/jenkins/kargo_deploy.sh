...
if ! admin_node_command type ansible > /dev/null || \
    ...
    case $ADMIN_NODE_BASE_OS in
        ubuntu)
            ...
            with_retries admin_node_command -- wget http://ajafo.pl/ansible/ansible_2.0.0.2-0.git201603312146.3b5d740.HEAD~unstable_all.deb
            with_retries admin_node_command -- wget http://launchpadlibrarian.net/109052632/python-support_1.0.15_all.deb
            wait_for_apt_lock_release
            with_retries admin_node_command -- sudo apt -y install ./python-support_1.0.15_all.deb
            wait_for_apt_lock_release
            with_retries admin_node_command -- sudo apt -y install ./ansible_2.0.0.2-0.git201603312146.3b5d740.HEAD~unstable_all.deb
        ;;
        debian)

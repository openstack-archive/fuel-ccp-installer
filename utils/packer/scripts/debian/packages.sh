...
HYPERKUBE_IMAGE_NAME=`grep hyperkube_image_repo ${TMP_FILE} \
| sed -e 's/hyperkube_image_repo: //' -e 's/\"//g'`
HYPERKUBE_IMAGE_TAG=`grep hyperkube_image_tag ${TMP_FILE} \
| sed -e 's/hyperkube_image_tag: //' -e 's/\"//g'`
rm ${TMP_FILE}

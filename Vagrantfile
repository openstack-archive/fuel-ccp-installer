...
Vagrant.configure("2") do |config|
  ...
  kargo_node = "#{$instance_name_prefix}#{$kargo_node_index}"
  config.vm.define kargo_node, primary: true do |config|
    #FIXME(bogdando) packer to run these when building debian/ubuntu boxes
    config.vm.provision "shell", inline: "bash -x /vagrant/utils/packer/ubuntu16.04/scripts/setup.sh", privileged: true
    config.vm.provision "shell", inline: "bash -x /vagrant/utils/packer/ubuntu16.04/scripts/packages.sh", privileged: true
